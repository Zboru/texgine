	...
	<div>
		<t-modal icon-preset="danger" v-model="show">
			...
			<template #description>
				Are you sure you want to delete this game? Game and its data will be permanently removed. This action
				cannot be undone.
			</template>
			<template #actions>
				<t-button :block="isMobile" variant="danger" :class="deleteButtonClasses" @click="deleteGame">Delete
				</t-button>
...
export default {
	...
	methods: {
		...
		deleteGame() {
			httpManager.delete(`${import.meta.env.VITE_API_URL}/games/${this.game.id}`)
				.then(() => {
					this.$emit('deleted', this.game.id);
					this.alert = true;
					this.show = false;
				});
		},
