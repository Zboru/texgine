<template>
  <t-container class="grid grid-cols-2 h-5/6">
    <div class="px-3">
      ...
      <div class="mt-2">
        <template v-for="(game, index) in games">
          <game-bar :game="game" class="mt-1" :key="index"></game-bar>
        </template>
      </div>
    </div>
    <game-details class="px-3"></game-details>
  </t-container>
</template>
...
export default {
  ...
  created() {
    httpManager.get('http://localhost:1337/api/user/Zeu2e0b1RBZQCZllHv3VI89okkt2/games').then((response) => {
  ...
  data() {
    return {
      ...
      games: [],
      items: [
        { text: 'Most played', value: 'most_played' },
        { text: 'Most loved', value: 'most_loved' },
